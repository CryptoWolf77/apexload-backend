...
class InstagramSafetyService:
    ...
    def finish_success(self, decision: InstagramSafetyDecision | None = None) -> None:
        ...
        with self._lock:
            ...
            threshold = max(settings.instagram_recovery_success_threshold, 1)
            if previous_mode in {"paused", "limited"}:
                if int(self._state["consecutive_successes"]) >= threshold:
                    self._state.update(
                        mode="active",
                    ...
                    self._state.update(
                        mode="limited",
                        reason="Instagram recovery check succeeded. Waiting for another success.",
                        reason_category="recovery_check",
                        paused_until=None,
                    )
            else:
    ...
    def finish_failure(
        ...
        with self._lock:
            ...
            if cooldown is not None:
                ...
                self._send_alert_email_locked(classification)
            else:
                self._state.update(
                    mode="limited",
                    reason=classification.safe_user_message,
    ...
    def manual_check(self) -> dict[str, Any]:
        ...
        if result.get("is_valid"):
            ...
            self.finish_failure(result.get("technical_reason") or result.get("message") or result)
    ...
    def _cooldown_for(
        ...
        if (
            classification.category == "unknown_instagram_error"
            and failures >= max(settings.instagram_failure_threshold, 1)
        ):
            return timedelta(minutes=max(settings.instagram_unknown_error_cooldown_minutes, 1))
        return None

    def _send_alert_email_locked(self, classification: InstagramErrorClassification) -> None:
        settings = get_settings()
...
Reason category: {classification.category}
...
Suggested action:
{_suggested_action(classification.category)}
"""
        subject = (
